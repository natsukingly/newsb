class AutoScrollController < ApplicationController
	...
	def load_articles
		...
		@loaded_articles = Article.all.where(country_id: @country.id).where("published_time >= ?", Time.zone.now.ago(5.days)).order(e_indecator: :desc, published_time: :desc).offset(@existing_articles).limit(30)
	end
	
	def load_categorized_articles
		#featured_article = 1
		@existing_articles = params[:existing_articles].to_i + 1
		@loaded_articles = Article.where(country_id: @country.id, category_id: params[:category_id]).where("published_time >= ?", Time.zone.now.ago(5.days)).order(e_indecator: :desc, published_time: :desc).offset(@existing_articles).limit(30)
	end
