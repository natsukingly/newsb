class CategoriesController < ApplicationController
	...
	def top
		...
		@articles = Article.all.where(category_id: Category.all.pluck(:id), country_id: @country.id).where("published_time >= ?", Time.now.ago(5.days)).where.not(posts_count: 0).order(e_indecator: :desc, published_time: :desc).limit(30)
